function [allPosPix dircs ]= computeMotion_inSquare_BM(trialAngle,dotCoh,aperturePix)
global params; 
...
for i = 2:params.stim.durInFrames
    ...
    %change here to have half the dots move in one direction
    if i > numFrames100
...
allPosPix.x = floor((params.screenVar.ppd .* allPos.x)+ aperturePix(1));%centerPix(1); 
allPosPix.y = floor((params.screenVar.ppd .* allPos.y)+ aperturePix(2));	